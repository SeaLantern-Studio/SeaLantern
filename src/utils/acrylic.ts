...
import { supportsNativeWindowMaterial } from "./platform";
...
export function applyAcrylicEffect(enabled: boolean): void {
  ...
  // 无原生材质的平台(如 Linux):仅组件层做 CSS 半透明模糊
  if (enabled && !supportsNativeWindowMaterial()) {
    root.setAttribute("data-material-fallback", "css-glass");
  } else {
    root.removeAttribute("data-material-fallback");
  }
