  ...
  <div class="anticheat-view">
        ...
        <div v-else class="detection-list">
          ...
          >
            <div class="detection-main">
              ...
              <span
                class="risk-badge"
                :style="{ backgroundColor: getRiskLevel(detection.risk_score).color }"
              >
    ...
    <div v-if="activeTab === 'settings'" class="tab-content">
      <div class="section-card">
        ...
        <button class="save-btn" @click="saveSettings"><Save /> {{ t("ai.common.save") }}</button>
      </div>
    </div>
        ...
        <div class="modal-body">
          <div class="detail-grid">
            ...
            <div class="detail-item">
              ...
              <span
                class="risk-score"
                :style="{ color: getRiskLevel(selectedDetection.risk_score).color }"
              >
                {{ selectedDetection.risk_score }} ({{
                  getRiskLevel(selectedDetection.risk_score).level
                }})
              </span>
            </div>
            <div class="detail-item">
...
const filteredDetections = computed(() => {
  return detections.value.filter((d) => {
    if (
      ...
      !d.player_name.toLowerCase().includes(filterPlayerName.value.toLowerCase())
    ) {
      return false;
    }
...
async function executeAction(detection: DetectionRecord, actionType: "kick" | "ban") {
  const action: PunishmentAction =
    actionType === "kick" ? { type: "kick" } : { type: "permanent_ban" };

  if (
    confirm(
      `确定要对玩家 ${detection.player_name} 执行${actionType === "kick" ? "踢出" : "封禁"}操作吗？`,
    )
  ) {
