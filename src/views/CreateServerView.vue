...
const isCoreDownloading = computed(
  () => coreDownloadLaunching.value || (coreDownloadTaskInfo.id !== "" && !coreDownloadTaskInfo.isFinished),
);
...
watch([coreDownloadTargetPath, coreDownloadCoreType, coreDownloadMcVersion], ([target, coreType, mcVersion], previous) => {
...
async function handleCoreDownload() {
  clearError();
  if (!coreDownloadTargetPath.value.trim() || !coreDownloadCoreType.value || !coreDownloadMcVersion.value) {
  ...
  try {
    ...
    const sameDownloadFolder =
      normalizePathForMatch(normalizedTargetPath) === normalizePathForMatch(lastCoreDownloadTargetPath.value);
...
<template>
  <div class="create-view animate-fade-in-up">
    ...
    <SLCard class="create-stepper-card" :title="i18n.t('create.title')">
      <StepperRoot orientation="vertical" :model-value="currentStep" :linear="false" class="create-stepper">
        <StepperItem
        ...
        >
          ...
          <div v-if="item.step === currentStep" class="create-step-panel">
                ...
                <div class="create-download-card-form">
                  ...
                  <div class="create-download-card-selects">
                    <div class="create-download-card-field">
                      <p class="create-download-card-label">{{ i18n.t("create.source_method_two_core_type") }}</p>
                      <SLSelect
                    ...
                    <div class="create-download-card-field">
                      <p class="create-download-card-label">{{ i18n.t("create.source_method_two_game_version") }}</p>
                      <SLSelect
                  ...
                  <div class="create-download-card-action">
                    ...
                    />
                    <p v-else class="create-download-done">{{ i18n.t("create.source_method_two_done") }}</p>
                  </div>
                </div>
              </div>
            </template>
            ...
            <div v-if="currentStep < 5" class="create-nav-actions">
              <SLButton variant="secondary" size="lg" :disabled="currentStep === 1" @click="goPrevStep">
