...
import { ChevronDown, ChevronUp, Minus, Square, X } from "lucide-vue-next";
...
async function closeWindow() {
  // macOS 默认最小化到后台，除非用户明确设置为关闭
  if (isMac.value) {
    if (closeAction.value === "close") {
      await appWindow.close();
    } else {
      // macOS 默认行为：最小化到后台（包括 "ask" 和 "minimize"）
      await minimizeToTray();
    }
  } else {
    // Windows/Linux 保持原有逻辑
...
<template>
  <header class="app-header glass-subtle" :class="{ 'app-header-macos': isMac }">
    <div class="header-left" :class="{ 'header-left-macos': isMac }">
...
/* macOS 标题栏样式：红绿灯已在单独的标题栏区域，不需要额外 padding */
.app-header-macos {
