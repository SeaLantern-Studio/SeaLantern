...
import { Github, Layers, MoreVertical, RefreshCw, Settings, ShieldAlert } from "lucide-vue-next";
  ...
  <div>
    <div v-if="batchMode" class="batch-action-bar">
      <div class="batch-action-left">
        <span class="selected-count">{{
          i18n.t("plugins.selected_count", { count: selectedPluginIds.size })
        }}</span>
      </div>
    ...
    <div class="plugin-grid">
      <SLCard
        ...
        class="plugin-card"
        :class="{ 'plugin-card--selected': batchMode && selectedPluginIds.has(plugin.manifest.id) }"
      >
        <div class="plugin-content">
          <label v-if="batchMode" class="plugin-checkbox" @click.stop>
            <SLCheckbox
              :modelValue="selectedPluginIds.has(plugin.manifest.id)"
              @update:modelValue="emit('toggle-plugin-selection', plugin.manifest.id)"
            />
          </label>

          <div class="plugin-card-actions">
            <div
              v-if="updates[plugin.manifest.id]"
              class="update-badge"
          ...
          <div class="plugin-main">
            <div class="plugin-icon">
              <img
                v-if="icons[plugin.manifest.id]"
                :src="icons[plugin.manifest.id]"
                alt="plugin icon"
