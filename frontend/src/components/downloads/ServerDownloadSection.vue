...
const summaryFacts = computed(() => [
  {
    label: i18n.t("downloads.next.server.summary.target"),
    value: props.savePath || i18n.t("downloads.next.server.summary.not_set"),
  },
  {
    label: i18n.t("downloads.next.server.summary.version"),
    value: props.selectedVersion || i18n.t("downloads.next.server.summary.not_set"),
  },
...
<template>
  <div class="server-download-section">
    <WorkbenchPanel
      :title="i18n.t('downloads.next.server.form_title')"
      :description="i18n.t('downloads.next.server.form_description')"
    >
      <div class="server-download-section__form-grid">
        <div class="server-download-section__field">
          ...
          <SLSelect
            ...
            searchable
            @update:model-value="emit('updateSelectedType', String($event))"
          />
        </div>
        <div class="server-download-section__field">
          ...
          <SLSelect
            ...
            :disabled="loadingVersions || !selectedType || submitting"
            searchable
            @update:model-value="emit('updateSelectedVersion', String($event))"
          />
        </div>
        <div class="server-download-section__field server-download-section__field--full">
          ...
          <div class="server-download-section__picker">
            ...
            <SLButton
              ...
              >{{ i18n.t("downloadServerView.actions.pickFolder") }}</SLButton
            >
          </div>
        </div>
        <div class="server-download-section__field">
    ...
    <WorkbenchPanel
      :title="i18n.t('downloads.next.server.summary_title')"
      :description="i18n.t('downloads.next.server.summary_description')"
    >
      ...
      <div class="server-download-section__actions">
        <SLButton
          ...
          :disabled="!canSubmit"
          @click="emit('submit')"
          >{{ i18n.t("downloads.next.server.submit") }}</SLButton
        >
      </div>
    </WorkbenchPanel>
  </div>
...
.server-download-section__form-grid {
  display: grid;
  grid-template-columns: repeat(2, minmax(0, 1fr));
  gap: 14px;
}
.server-download-section__field {
  min-width: 0;
  display: grid;
  gap: 6px;
}
.server-download-section__field--full {
  grid-column: 1 / -1;
}
.server-download-section__field label {
  font-size: 0.82rem;
  color: var(--sl-text-tertiary);
  font-weight: 500;
}
.server-download-section__picker {
  display: flex;
  align-items: center;
...
.server-download-section__picker-copy {
  min-width: 0;
  display: flex;
  align-items: center;
  gap: 10px;
}
.server-download-section__picker-copy div,
.server-download-section__summary-extra {
  min-width: 0;
  display: grid;
  gap: 4px;
}
.server-download-section__picker-copy strong,
.server-download-section__picker-copy span,
.server-download-section__summary-extra strong,
.server-download-section__summary-extra span {
  margin: 0;
}
.server-download-section__picker-copy strong,
.server-download-section__summary-extra strong {
...
.server-download-section__picker-icon {
  flex: none;
  color: var(--sl-primary);
}
.server-download-section__actions {
  display: flex;
