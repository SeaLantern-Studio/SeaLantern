...
<template>
  <WorkbenchPanel
    tone="danger"
    :title="i18n.t('developer.next.other.title')"
    :description="i18n.t('developer.next.other.description')"
  >
    <WorkbenchStatusBanner tone="error">{{
      canTriggerCrash
        ? i18n.t("developer.next.other.warning")
    ...
    <div class="developer-danger-section__group">
      ...
      <div class="developer-danger-section__actions">
        <SLButton
          ...
          @click="emit('triggerCrash')"
          >{{ i18n.t("developer.crash_test_button") }}</SLButton
        >
      </div>
    </div>
  </WorkbenchPanel>
...
<style scoped>
.developer-danger-section__group {
  display: grid;
  gap: 10px;
  padding: 14px 16px;
  border-radius: 18px;
  border: 1px solid color-mix(in srgb, var(--sl-error) 24%, var(--sl-border));
  background: color-mix(in srgb, var(--sl-error) 6%, var(--sl-bg-secondary));
}
.developer-danger-section__copy {
  display: grid;
  gap: 4px;
}
.developer-danger-section__copy strong,
.developer-danger-section__copy p {
  margin: 0;
}
.developer-danger-section__copy p {
...
.developer-danger-section__actions {
  display: flex;
