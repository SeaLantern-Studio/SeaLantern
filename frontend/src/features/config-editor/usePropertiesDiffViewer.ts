...
} from "@src/features/config-editor/propertiesCodeMirror";
import { buildDiffLines, type DiffLine, type DiffLineType } from "@utils/configDiff";

interface UsePropertiesDiffViewerOptions {
...
export function usePropertiesDiffViewer(options: UsePropertiesDiffViewerOptions) {
  ...
  function createEditor() {
    ...
    const lines = buildContextDiffLines(
      buildDiffLines(options.original.value, options.modified.value),
      3,
    );
  ...
  watch([options.original, options.modified], async () => {
    await nextTick();
    createEditor();
  });
