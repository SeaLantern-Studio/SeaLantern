        ...
        while let Some(event) = events.recv().await {
            let message = match &event {
                TunnelEvent::PlayerJoined { id } => {
                    tunnel_t1("tunnel.log.player_joined", id.to_string())
                }
                ...
                TunnelEvent::AuthFailed { id } => {
                    tunnel_t1("tunnel.log.auth_failed", id.to_string())
                }
                ...
                TunnelEvent::Error { message } => {
                    tunnel_t1("tunnel.log.error_event", message.clone())
                }
