...
├── server-config/            启动配置
├── docker/                   Docker 规则
├── server-local-setup/       本地服务器接管
├── server-installer/         服务端安装
├── server-startup-scan/      启动候选扫描
├── java-installer/           Java 安装
├── server-log/               服务器日志
├── server-plugin/            服务端插件文件
├── plugin-trust/             插件信任
├── lua-runtime/              Lua 边界
├── server-download-links/    下载链接解析
...
| 更新命令 | `tauri-host/src/commands/update/`、`update/` |
| 后端服务编排 | `tauri-host/src/services/` |
| 桌面 / headless 宿主行为 | `tauri-host/` |
| Headless HTTP、运行模式、端口、panic report | `runtime/`、`tauri-host/src/runtime/` |
| CLI server / compose / docker doctor | `tauri-host/src/utils/cli/` |
| Docker 服务器启动 | `docker/`、`tauri-host` 中调用 Docker 的服务 |
| Docker / headless 独立入口 | `docker-entry/` |
| 本地 Java 服务器启动 | `server-local-setup/`、`server-config/`、`tauri-host/src/services/server/manager/runtime_start/` |
| 服务器配置和 JVM 参数 | `server-config/` |
| 创建、导入、modpack provisioning | `tauri-host/src/services/server/manager/provisioning/`、`server-installer/`、`server-local-setup/` |
| 服务端安装和核心识别 | `server-installer/`、`server-download-links/`、`starter-links/`、`tauri-host/src/services/download/` |
| 扫描已有服务器目录 | `server-startup-scan/` |
| Java 下载和安装 | `java-installer/` |
| 日志持久化和读取 | `server-log/`、`tauri-host` 输出读取逻辑 |
| 应用事件和服务器事件 | `event/`、`tauri-host` 事件转发 |
| 后端多语言 | `i18n/` |
| Sea Lantern Lua 插件运行时 | `tauri-host/src/plugins/`、`lua-runtime/`、`plugin-trust/` |
| 插件 UI snapshot、sidebar、context menu、component mirror | `tauri-host/src/plugins/api/`、`tauri-host/src/plugins/runtime/ui/` |
| Sea Lantern 插件权限和信任 | `plugin-trust/` |
| Minecraft 服务端插件文件 | `server-plugin/` |
| 在线服务、隧道、OneBot | `tauri-host/src/services/online/`、`tauri-host/src/commands/online/` |
| 更新检查、下载和安装 | `update/`、`tauri-host` 更新命令 |
...
- 如果 preview 和真实执行都需要同一套规则，优先把归一化逻辑放到共享 crate。
...
- `server-config` 负责启动配置、JVM 参数、CPU policy 和 `server.properties` 规则。
- `server-local-setup` 负责本地目录检查、启动模式推断、Java 路径和本地启动预览辅助。
...
- Docker CPU policy 应和 `server-config` 保持一致，除非明确要拆分行为。
- `docker` 不负责实际执行 Docker 进程，也不负责宿主状态流转。
...
- `runtime` 负责共享 runtime 工具，不应放 Minecraft 业务规则。

### 插件

- Sea Lantern Lua 插件系统和 Minecraft 服务端插件文件管理是两套逻辑。
- Lua 插件权限、信任和权限归一化应集中在 `plugin-trust`。
- Lua runtime trait 签名是插件运行时边界，改动时要同步 host 和 runtime。
- Minecraft 服务端插件 jar / 配置扫描应留在 `server-plugin`。
...
- Put reusable rules in the owning shared crate.
