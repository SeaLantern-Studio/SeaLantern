...
pub(crate) const DEFAULT_CONTAINER_UPLOAD_DIR: &str = "/app/uploads";
...
const LOCAL_UPLOAD_DIR_NAME: &str = "uploads";
...
pub const UPLOAD_DIR_ENV: &str = "SEALANTERN_UPLOAD_DIR";
...
fn default_upload_dir() -> PathBuf {
    if let Some(value) = env_var_trimmed(UPLOAD_DIR_ENV) {
        return PathBuf::from(value);
    }

    if Path::new("/.dockerenv").exists() {
        return PathBuf::from(DEFAULT_CONTAINER_UPLOAD_DIR);
    }

    crate::get_app_data_dir().join(LOCAL_UPLOAD_DIR_NAME)
}
...
mod tests {
    use super::{
        ...
        DEFAULT_CONTAINER_UPLOAD_DIR, DEFAULT_MAX_UPLOAD_BYTES, DEFAULT_MAX_UPLOAD_FILES,
        DEFAULT_MAX_UPLOAD_FILE_BYTES, HTTP_AUTH_TOKEN_ENV, HTTP_CORS_ORIGINS_ENV, UPLOAD_DIR_ENV,
        WEB_AUTH_RECOVERY_TOKEN_ENV,
    };
    ...
    fn default_headless_http_config_uses_expected_defaults() {
        ...
        let _upload_guard = EnvGuard::remove(UPLOAD_DIR_ENV);

        let expected_upload_dir = if Path::new("/.dockerenv").exists() {
            PathBuf::from(DEFAULT_CONTAINER_UPLOAD_DIR)
        } else {
            crate::get_app_data_dir().join("uploads")
        };

        let config = default_headless_http_config();
        assert!(!config.auth_token.is_empty());
        assert_eq!(config.upload_dir, expected_upload_dir);
    ...
    #[test]
    fn default_headless_http_config_accepts_upload_dir_override() {
        let _lock = lock_env();
        let _upload_guard = EnvGuard::set(UPLOAD_DIR_ENV, "/tmp/sealantern-test-uploads");

        let config = default_headless_http_config_checked().expect("config should build");

        assert_eq!(config.upload_dir, PathBuf::from("/tmp/sealantern-test-uploads"));
    }
