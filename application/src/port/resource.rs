...
/// # `save_path` 语义与清理责任
///
/// `save_path` 指向应用数据根目录 `market-tmp/<项目>-<版本>-<随机后缀>/` 下的
/// 唯一子目录路径，**文件名保持市场原名**（唯一化只作用于目录，因此安装进
/// 实例后 `mods/` 里就是 `sodium.jar`）。随机后缀保证同一项目与版本的并发
/// 解析各自拿到独立路径，不会互相覆盖。
///
/// 安装成功后由服务端**尽力**清理该临时子目录：清理失败**不影响安装结果**，
/// 仅记录告警日志，残留由服务端按年龄回收。前端无需自行清理，也不要改动路径。
