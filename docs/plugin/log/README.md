---
title: sl.log Namespace
description: "本文档说明插件运行时暴露的 [`sl.log`](../../backend/tauri-host/src/plugins/runtime/log.rs) Lua 接口，用于向 SeaLantern 运行时输出插件日志，并同步发送日志事件。"
...
- [sl.log.debug()](./debug.md): 输出调试日志；当插件缺少日志权限时，该调用会静默忽略
- [sl.log.info()](./info.md): 输出信息日志，并发送对应日志事件
- [sl.log.warn()](./warn.md): 输出警告日志，并发送对应日志事件
- [sl.log.error()](./error.md): 输出错误日志，并发送对应日志事件
