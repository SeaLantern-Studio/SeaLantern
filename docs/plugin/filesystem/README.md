---
title: sl.fs Namespace
description: "本文档说明插件运行时暴露的 [`sl.fs`](../../backend/tauri-host/src/plugins/runtime/filesystem.rs) Lua 接口，用于在受限 sandbox 内执行文件读写、目录管理、元信息查询与条目转移操作。"
...
- [sl.fs.read(scope, path)](./read.md): 读取文本文件内容
- [sl.fs.read_binary(scope, path)](./read-binary.md): 读取二进制文件并返回 Base64 字符串
- [sl.fs.exists(scope, path)](./exists.md): 判断文件或目录是否存在
- [sl.fs.list(scope, path)](./list.md): 列出目录下的直接子项名称
- [sl.fs.info(scope, path)](./info.md): 获取条目元信息，包含 `size`、`is_dir`、`modified`
- [sl.fs.get_path(scope)](./get-path.md): 获取虚拟 sandbox 路径标识，而非宿主真实路径
- [sl.fs.write(scope, path, content)](./write.md): 写入文本内容，不存在的父目录会自动创建
- [sl.fs.mkdir(scope, path)](./mkdir.md): 创建目录，等价于递归创建
- [sl.fs.remove(scope, path)](./remove.md): 删除文件或空目录；拒绝递归删除非空目录与 sandbox 根
- [sl.fs.copy(scope, src, dst)](./copy.md): 在同一 scope 内复制文件或目录；目标已存在时拒绝覆盖
- [sl.fs.move(scope, src, dst)](./move.md): 在同一 scope 内移动文件或目录
- [sl.fs.rename(scope, oldPath, newPath)](./rename.md): 在同一 scope 内重命名文件或目录
