---
title: sl.http Namespace
description: "本文档说明插件运行时暴露的 [`sl.http`](../../backend/tauri-host/src/plugins/runtime/http/mod.rs) Lua 接口，用于在插件中发起受限的 HTTP 网络请求访问外部互联网资源。"
...
- [sl.http.get(url, options?)](./get.md): 发起 `GET` 请求，请求成功时返回响应表，失败时返回 `nil, error`
- [sl.http.post(url, body?, options?)](./post.md): 发起 `POST` 请求，`body` 为 Lua 表时会自动序列化为 JSON
- [sl.http.put(url, body?, options?)](./put.md): 发起 `PUT` 请求，`body` 为 Lua 表时会自动序列化为 JSON
- [sl.http.delete(url, options?)](./delete.md): 发起 `DELETE` 请求
