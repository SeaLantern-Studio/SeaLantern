...
- `sl.element.get_text(selector)`: 获取匹配元素的文本内容；查询失败、超时或未找到时返回 `nil`
- `sl.element.get_value(selector)`: 获取输入类元素的当前值；查询失败、超时或未找到时返回 `nil`
- `sl.element.exists(selector)`: 判断元素是否存在，返回字符串 `"true"` / `"false"`，超时返回 `nil`
- `sl.element.is_visible(selector)`: 判断元素是否可见，返回字符串 `"true"` / `"false"`，超时返回 `nil`
- `sl.element.is_enabled(selector)`: 判断元素是否启用，返回字符串 `"true"` / `"false"`，超时返回 `nil`
- `sl.element.get_attribute(selector, attr)`: 获取元素指定属性值；查询失败、超时或未找到时返回 `nil`
- `sl.element.get_attributes(selector)`: 获取元素全部属性，返回 Lua table；查询失败或超时时返回 `nil`
- `sl.element.click(selector)`: 触发元素点击行为
- `sl.element.set_value(selector, value)`: 设置输入类元素的值，并派发 `input` / `change` 事件
- `sl.element.check(selector, checked)`: 设置复选框 / 单选框选中状态，并派发 `change` 事件
- `sl.element.select(selector, value)`: 设置下拉框选中值，并派发 `change` 事件
- `sl.element.set_attribute(selector, attr, value)`: 批量设置匹配元素的单个属性，`nil` 会被写为空字符串
- `sl.element.set_style(selector, styles)`: 批量设置匹配元素样式，支持 Lua table 转 JSON 后下发前端
- `sl.element.form_fill(selector, fields)`: 按字段名批量填写表单，支持 `input` / `textarea` / `select` / 复选框组
- `sl.element.focus(selector)`: 让元素获得焦点
- `sl.element.blur(selector)`: 让元素失去焦点
- `sl.element.on_change(selector, callback)`: 监听元素 `change` 事件，返回 cleanup 函数用于解除监听
