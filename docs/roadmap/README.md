...
- Rust workspace 当前包含 `application`、`core`、`extra`、`infra`、`interface`、`server`、`src-tauri` 以及两个独立许可的 vendored crate。
...
| P1     | 契约层边界收敛             | `crates/interface` 的共享 DTO/服务契约不再无必要地扩大对 `core`/`extra` 实现模型的依赖 |
