...
| ----------------------------------------------- | ----------------------------------------------------- | ----------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------- |
| `sl.server.list()`                              | 无                                                    | `table<number, table>`  | 列出当前已注册的服务器实例                                 | [`files::list()`](../../backend/tauri-host/src/plugins/runtime/server/files.rs:8)        |
| `sl.server.get_path(serverId)`                  | `serverId: string`                                    | `string`                | 获取指定服务器的根目录路径                                 | [`files::get_path()`](../../backend/tauri-host/src/plugins/runtime/server/files.rs:22)   |
| `sl.server.read_file(serverId, path)`           | `serverId: string`，`path: string`                    | `string`                | 读取服务器目录中的文本文件                                 | [`files::read_file()`](../../backend/tauri-host/src/plugins/runtime/server/files.rs:31)  |
| `sl.server.write_file(serverId, path, content)` | `serverId: string`，`path: string`，`content: string` | `boolean`               | 向服务器目录写入文本文件，不存在的父目录会自动创建         | [`files::write_file()`](../../backend/tauri-host/src/plugins/runtime/server/files.rs:49) |
| `sl.server.list_dir(serverId, path)`            | `serverId: string`，`path: string`                    | `table<number, table>`  | 列出目录下的直接子项及其基础元信息                         | [`files::list_dir()`](../../backend/tauri-host/src/plugins/runtime/server/files.rs:75)   |
| `sl.server.exists(serverId, path)`              | `serverId: string`，`path: string`                    | `boolean`               | 判断服务器目录中的文件或目录是否存在                       | [`files::exists()`](../../backend/tauri-host/src/plugins/runtime/server/files.rs:120)    |
| `sl.server.logs.get(serverId, count?)`          | `serverId: string`，`count?: integer`                 | `table<number, string>` | 获取指定服务器最近 N 条日志，默认 `100`，最大 `1000`       | [`get()`](../../backend/tauri-host/src/plugins/runtime/server/logs.rs:19)                |
| `sl.server.logs.getAll(count?)`                 | `count?: integer`                                     | `table<number, table>`  | 获取所有运行中服务器最近 N 条日志，默认 `100`，最大 `1000` | [`get_all()`](../../backend/tauri-host/src/plugins/runtime/server/logs.rs:37)            |
...
每个服务器条目包含以下字段，构造逻辑见 [`create_server_entry()`](../../backend/tauri-host/src/plugins/runtime/server/common.rs:120)：
...
该接口只适用于文本内容读取，读取前会执行文件大小检查，限制见 [`MAX_FILE_SIZE`](../../backend/tauri-host/src/plugins/runtime/server/common.rs:10)。
...
如果目标父目录不存在，会自动创建，具体逻辑见 [`files::write_file()`](../../backend/tauri-host/src/plugins/runtime/server/files.rs:49)。
