...
| --------------------------------------- | ----------------------------------------------------- | ----------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| `sl.fs.read(scope, path)`               | `scope: string`，`path: string`                       | `string`                | 读取文本文件内容                                    | [`read::read()`](../../backend/tauri-host/src/plugins/runtime/filesystem/read.rs:9)                  |
| `sl.fs.read_binary(scope, path)`        | `scope: string`，`path: string`                       | `string`                | 读取二进制文件并返回 Base64 字符串                  | [`read::read_binary()`](../../backend/tauri-host/src/plugins/runtime/filesystem/read.rs:31)          |
| `sl.fs.exists(scope, path)`             | `scope: string`，`path: string`                       | `boolean`               | 判断文件或目录是否存在                              | [`read::exists()`](../../backend/tauri-host/src/plugins/runtime/filesystem/read.rs:58)               |
| `sl.fs.list(scope, path)`               | `scope: string`，`path: string`                       | `table<number, string>` | 列出目录下的直接子项名称                            | [`read::list()`](../../backend/tauri-host/src/plugins/runtime/filesystem/read.rs:78)                 |
| `sl.fs.info(scope, path)`               | `scope: string`，`path: string`                       | `table`                 | 获取条目元信息，包含 `size`、`is_dir`、`modified`   | [`read::info()`](../../backend/tauri-host/src/plugins/runtime/filesystem/read.rs:110)                |
| `sl.fs.get_path(scope)`                 | `scope: string`                                       | `string`                | 获取虚拟 sandbox 路径标识，而非宿主真实路径         | [`read::get_path()`](../../backend/tauri-host/src/plugins/runtime/filesystem/read.rs:144)            |
| `sl.fs.write(scope, path, content)`     | `scope: string`，`path: string`，`content: string`    | `nil`                   | 写入文本内容，不存在的父目录会自动创建              | [`write::write()`](../../backend/tauri-host/src/plugins/runtime/filesystem/write.rs:8)               |
| `sl.fs.mkdir(scope, path)`              | `scope: string`，`path: string`                       | `nil`                   | 创建目录，等价于递归创建                            | [`write::mkdir()`](../../backend/tauri-host/src/plugins/runtime/filesystem/write.rs:38)              |
| `sl.fs.remove(scope, path)`             | `scope: string`，`path: string`                       | `nil`                   | 删除文件或空目录；拒绝递归删除非空目录与 sandbox 根 | [`write::remove()`](../../backend/tauri-host/src/plugins/runtime/filesystem/write.rs:63)             |
| `sl.fs.copy(scope, src, dst)`           | `scope: string`，`src: string`，`dst: string`         | `nil`                   | 在同一 scope 内复制文件或目录；目标已存在时拒绝覆盖 | [`transfer::copy()`](../../backend/tauri-host/src/plugins/runtime/filesystem/transfer.rs:8)          |
| `sl.fs.move(scope, src, dst)`           | `scope: string`，`src: string`，`dst: string`         | `nil`                   | 在同一 scope 内移动文件或目录                       | [`transfer::move_entry()`](../../backend/tauri-host/src/plugins/runtime/filesystem/transfer.rs:47)   |
| `sl.fs.rename(scope, oldPath, newPath)` | `scope: string`，`oldPath: string`，`newPath: string` | `nil`                   | 在同一 scope 内重命名文件或目录                     | [`transfer::rename_entry()`](../../backend/tauri-host/src/plugins/runtime/filesystem/transfer.rs:77) |
