...
| ---------------- | ----------------- | ------ | ---------------------------------------------------- | ---------------------------------------------------------------------------- |
| `sl.log.debug()` | `message: string` | `nil`  | 输出调试日志；当插件缺少日志权限时，该调用会静默忽略 | [`create_log_function()`](../../backend/tauri-host/src/plugins/runtime/log/emit.rs:4) |
| `sl.log.info()`  | `message: string` | `nil`  | 输出信息日志，并发送对应日志事件                     | [`create_log_function()`](../../backend/tauri-host/src/plugins/runtime/log/emit.rs:4) |
| `sl.log.warn()`  | `message: string` | `nil`  | 输出警告日志，并发送对应日志事件                     | [`create_log_function()`](../../backend/tauri-host/src/plugins/runtime/log/emit.rs:4) |
| `sl.log.error()` | `message: string` | `nil`  | 输出错误日志，并发送对应日志事件                     | [`create_log_function()`](../../backend/tauri-host/src/plugins/runtime/log/emit.rs:4) |
...
当插件拥有日志权限时，会输出一条 `DEBUG` 级别日志；否则该调用会直接返回，不产生任何输出。权限分支注册逻辑见 [`PluginRuntime::setup_log_namespace()`](../../backend/tauri-host/src/plugins/runtime/log/mod.rs:10)。
...
该接口适合记录普通运行信息。日志消息会先转换为 Rust 字符串，再执行输出与事件发送，转换逻辑见 [`convert_lua_string()`](../../backend/tauri-host/src/plugins/runtime/log/common.rs:20)。
