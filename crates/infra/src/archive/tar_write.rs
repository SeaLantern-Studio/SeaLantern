...
/// uid/gid 归零，子目录按名称排序遍历。同一份源目录内容因此产出逐字节一致的
/// 归档，便于校验与去重。
///
...
pub fn create_tar_gz(
    ...
    let result = create_tar_gz_inner(source, destination);
...
fn create_tar_gz_inner(source: &Path, destination: &Path) -> Result<ArchiveSummary, ArchiveError> {
    ...
    match write_archive(&source_root, source, &temporary) {
        Ok(summary) => {
