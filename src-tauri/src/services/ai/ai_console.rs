...
use super::{
    ai_provider::get_provider, AIAnalysisResult, AIConfig, AIMessage, AIProvider, AIResponse,
};
...
impl AIConsoleService {
    /// 创建新的控制台助手服务
    pub fn new() -> Self {
        Self { patterns: get_predefined_patterns() }
    }
    ...
    async fn ai_analysis(
        ...
        let logs_to_analyze: Vec<&str> = logs
            .iter()
            .rev()
            .take(max_lines)
            .map(|s| s.as_str())
    ...
    /// 解释日志行
    pub async fn explain_log_line(&self, log_line: &str, config: &AIConfig) -> Option<String> {
        if !config.enabled {
            return None;
        }
        ...
        let messages = vec![
            AIMessage {
                role: "system".to_string(),
                content: "你是一个 Minecraft 服务器日志分析专家。请用简洁的语言解释日志内容。"
                    .to_string(),
                timestamp: chrono::Utc::now().timestamp(),
            },
            AIMessage {
