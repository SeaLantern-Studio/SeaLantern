...
impl m_PluginManager {
                ...
                if file_name.ends_with(".jar") || file_name.ends_with(".jar.disabled") {
                    ...
                    match self.m_parse_plugin_jar(&path) {
                        Ok(plugin_config) => {
                            ...
                            let plugin_name = plugin_config.name.clone().unwrap_or_else(|| base_file_name.replace(".jar", ""));
                            ...
                            plugins.push(m_PluginInfo {
                                m_id: format!("{}-{}", plugin_config.name.as_deref().unwrap_or("unknown"), plugin_config.version.as_deref().unwrap_or("unknown")),
                                name: plugin_config.name.unwrap_or_else(|| "Unknown Plugin".to_string()),
                                version: plugin_config.version.unwrap_or_else(|| "Unknown".to_string()),
                                description: plugin_config.description.unwrap_or_else(|| "No description".to_string()),
                                author,
    ...
    pub fn m_toggle_plugin(&self, server_path: &str, file_name: &str, enabled: bool) -> Result<(), String> {
        let plugins_dir = Path::new(server_path).join("plugins");
        ...
        if current_path.exists() {
            fs::rename(&current_path, &new_path).map_err(|e| format!("Failed to toggle plugin: {}", e))?;
        }
    ...
    pub async fn m_install_plugin(&self, server_path: &str, file_data: Vec<u8>, file_name: &str) -> Result<(), String> {
        let plugins_dir = Path::new(server_path).join("plugins");
    ...
    fn m_parse_plugin_jar(&self, jar_path: &Path) -> Result<m_PluginConfig, String> {
        ...
        for i in 0..zip.len() {
            ...
            if file.name() == "plugin.yml" || file.name() == "bungee.yml" {
                ...
                let config: m_PluginConfig = serde_yaml::from_str(&content).map_err(|e| format!("Failed to parse config file: {}", e))?;
                return Ok(config);
            }
        }
