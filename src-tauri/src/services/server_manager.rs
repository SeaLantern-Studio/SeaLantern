...
impl ServerManager {
    ...
    // 配置更新 处理
    pub fn update_server_config(&self, id: &str, new_max_mem: u32, new_min_mem: u32, new_jvm_args: Vec<String>) -> Result<(), String> {
        // 先搞进程 看看在不在
        let mut proc_map = self.processes.lock().expect("lock poisoned");
        let is_proc_running = proc_map.contains_key(id);
        drop(proc_map);
        // 跑着的话就停 等停完
        if is_proc_running {
    ...
    loop {
        ...
        if target_server.is_none() {
            return Err("服务器没找到".to_string());
        }
        let s = target_server.unwrap();
        s.max_memory = new_max_mem;
        s.min_memory = new_min_mem;
        s.jvm_args = new_jvm_args;
        // 解锁存一下
        drop(servers);
        self.save();
        Ok(())
...
.ok()
.and_then(|content| serde_json::from_str::<Vec>(&content).ok())
.unwrap_or_default()
}
