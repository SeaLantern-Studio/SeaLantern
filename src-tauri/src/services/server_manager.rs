...
impl ServerManager {
    ...
    pub fn import_server(&self, req: ImportServerRequest) -> Result<ServerInstance, String> {
        ...
        // 根据选项决定使用原有目录还是创建新目录
        let server_dir = if req.use_existing_dir {
            // 使用原有目录（启动文件所在的目录）
            source_startup_file
                .parent()
            ...
            // 创建新目录
            let new_dir = servers_dir.join(&id);
            std::fs::create_dir_all(&new_dir).map_err(|e| format!("无法创建服务器目录: {}", e))?;
            new_dir
        };
        ...
        let dest_startup: std::path::PathBuf = if req.use_existing_dir {
            // 使用原有目录时，直接使用原路径
            source_startup_file.to_path_buf()
        } else {
            // 创建新目录时，复制文件
            if startup_mode == "jar" {
        ...
        // 只有在创建新目录时才创建 server.properties
        if !req.use_existing_dir {
    ...
    pub fn start_server(&self, id: &str) -> Result<(), String> {
        ...
        // 确保 plugins 目录存在
        let plugins_dir = std::path::Path::new(&server.path).join("plugins");
        if !plugins_dir.exists() {
            std::fs::create_dir_all(&plugins_dir)
                .map_err(|e| format!("无法创建 plugins 目录: {}", e))?;
            println!("已创建 plugins 目录: {}", plugins_dir.display());
        }
