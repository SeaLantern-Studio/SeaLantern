...
impl ServerManager {
    pub fn new() -> Self {
        ...
        ServerManager {
            ...
            log_threads: Mutex::new(HashMap::new()),
        }
    }
    ...
    pub fn import_server(&self, req: ImportServerRequest) -> Result<ServerInstance, String> {
        ...
        // 检查目标文件是否已存在
        if dest_jar.exists() {
            return Err(format!("目标JAR文件已存在: {}", dest_jar.display()));
        }

        // 验证源文件在复制前仍然存在
        if !source_jar.exists() {
            return Err("源JAR文件在复制前已被删除".to_string());
        }
        ...
        // 检查server.properties是否已存在
        if server_properties_path.exists() {
            println!("server.properties已存在，将保留原文件");
        } else {
    ...
    pub fn start_server(&self, id: &str) -> Result<(), String> {
        ...
        {
            let mut threads = self.log_threads.lock().unwrap();
            threads.insert(id.to_string(), stop_flag.clone());
        }
        
        // 使用Arc安全共享数据
        let logs_arc = Arc::new(self.logs.clone());
        let logs_arc_clone = logs_arc.clone();
                                ...
                                match file.read_to_end(&mut buffer) {
                                    Ok(_) => {
                                        ...
                                        if let Ok(mut logs) = logs_arc_clone.lock() {
                                            if let Some(v) = logs.get_mut(&lid) {
                                    ...
                                    Err(_) => continue, // 读取失败就继续尝试
                                }
                            }
                            last_size = len;
                        }
                    }
                    Err(_) => continue, // 元数据读取失败就继续尝试
                }
            }
        });
    ...
    pub fn stop_server(&self, id: &str) -> Result<(), String> {
        // 停止日志读取线程
        if let Ok(mut threads) = self.log_threads.lock() {
            if let Some(stop_flag) = threads.remove(id) {
