...
async fn check_aur_update(current_version: &str) -> Result<UpdateInfo, String> {
    ...
    println!("release_notes 长度: {}", release_notes.len());
    println!("release_notes 内容: {}", release_notes);
    // ===== 调试输出结束 =====
...
pub async fn install_update(file_path: String, version: String) -> Result<(), String> {
    ...
    let result = (|| -> Result<(), String> {
        ...
        #[cfg(target_os = "linux")]
        {
            // Linux下：如果是AppImage才打开，否则提示
            if file_path.to_lowercase().ends_with(".appimage") {
                opener::open(&file_path).map_err(|e| format!("Failed to open update file: {}", e))?;
            } else {
                // 如果不是AppImage（如.deb/.rpm），让用户手动安装
                return Err(format!(
                    "下载的文件类型需要手动安装\n\
                     文件位置: {}\n\
                     请使用包管理器手动安装",
