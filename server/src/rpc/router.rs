...
use super::service::AppServices;

/// 组装当前所有已实现 RPC 方法的 Axum 路由。
///
/// 调用方创建 [`AppServices`] 容器并传入此函数，返回的 [`Router`] 可直接嵌套进
/// 更大的 Axum 应用，或通过 `tower::ServiceExt::oneshot` 测试。
///
/// # 示例
///
/// ```ignore
/// let services = AppServices::new(Arc::new(MyConsoleService));
/// let router = build_router(services, MyAccessResolver);
/// ```
pub fn build_router<A>(services: AppServices, access_resolver: A) -> Router
where
